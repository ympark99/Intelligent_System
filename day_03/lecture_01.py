...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 18 15:12:23 2022
...
#   머신러닝 알고리즘의 종류에 따라서 스케일 전처리가 필요함(중요)
print(X.describe())
...
# 일반적으로 데이터 전처리는 분할 전에 하지 않음(학습데이터에 대해서 수행)
...
# 데이터가 분할되는 결과를 가지기 때문에 학습의 성능은 올라가지만 실전에서 결과가 좋지 않은 모델이 생성될 수 있음


# 3. 데이터의 분할
# - 학습 데이터, 테스트 데이터 (머신러닝의 케이스)
# - 학습 데이터, 검증 데이터, 테스트 데이터 (딥러닝의 케이스)
#   (딥러닝의 경우 부분 배치 학습을 수행하여 점진적으로 학습량을 늘려나가는 경우가 많음)
...
#                                          train_size = 테스트데이터비율(0.2),
#                                           stratify=범주형데이터인경우만 y를 입력,
...
# (여러번 돌리면 결과 다르게 나옴)
# - 머신러닝의 알고리즘에 사용되는 하이퍼 파라메터를 테스트할때
...
# 확률 값으로 예측할 수 있음(일부 클래스에선 제공x)
# - predict_proba 메소드 사용
# - model.predict_proba(X - 설명qustn)
# - 예측확률의 기준선의 개념을 도입하여 일정 확률이상으로 예측하는 경우에만
# 예측의 결과를 수용하는방법을 사용
